...
@dp.callback_query_handler(text='view_refs_cm')
async def user_panel_view_refs_callback(callback_query: types.CallbackQuery):
    ...
    if not refs:
        await bot.send_message(chat_id=callback_query.from_user.id, text = f"You don't have any referrals yet", reply_markup=cm_panel_keyboard_back_to_main_menu)
    else:
        message = ''
        for ref in refs:
            message += f"{ref['user_id']}\n"
        await bot.send_message(chat_id=callback_query.from_user.id, text=message, reply_markup=cm_panel_keyboard_back_to_main_menu)